...
def dpsh(opt,
    ...
    for epoch in range(opt.epochs):
        ...
        if epoch % opt.evaluate_freq == opt.evaluate_freq - 1:
            ...
            logger.info('code_length: {}, epoch: {}, lr: {}, loss: {:.4f}, map: {:.4f}'.format(opt.code_length, epoch+1, scheduler.get_lr(),total_loss, meanAP))
...
def generate_code(model, dataloader, opt, np=False):
    """产生hash code
    ...
    with torch.no_grad():
        ...
        if np:
            code = np.zeros([N, opt.code_length])
        else:
        ...
        for data, _, index in tqdm(dataloader):
            ...
            outputs = model(data)
            if np:
                code[index, :] = outputs.sign().cpu().data.numpy()
            else:
