...
class BalancedBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for i in range(self.length):
            # print('555555555555555', len(self.keys), int(self.batch_size/self.batch_k))
...
if __name__ == '__main__':
    # temp = BalancedBatchSampler('./txt/train.txt', 128, 32)
    # temp = iter(temp)
    # for i in temp:
    #     print(i)

    traingen = TripletDataset('./txt/train.txt', transforms=transforms.Compose([transforms.ToTensor()]))
    temp = DataLoader(dataset=traingen,
                            batch_sampler=BalancedBatchSampler('./txt/train.txt', 128, 32),
                            num_workers=10,
                            pin_memory=False)
